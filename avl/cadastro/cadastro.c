...
void cadastrar_aluno(Lista **lista, ArvoreAVL *raiz_curso)
{
    ...
    if(curso != NULL)
    {
        ...
        aluno.arvavl_matricula = arvoreavl_cria();
        aluno.arvavl_nota = arvoreavl_cria();
...
void cadastrar_curso(ArvoreAVL **raiz_curso)
{
    ...
    curso.arvavl_disciplina = arvoreavl_cria();

    union Data info;
    info.curso = curso;

    if(arvoreavl_add(raiz_curso, info))
...
void cadastrar_disciplina(ArvoreAVL *raiz_curso)
{
    ...
    ArvoreAVL *curso;
    curso = arvoreavl_buscar(raiz_curso, codigo_curso);
    ...
    if(curso != NULL)
    {
        ...
        if(arvoreavl_add(&(curso->info.curso.arvavl_disciplina), info))
...
void cadastrar_matricula(Lista *lista, ArvoreAVL *raiz_curso)
{
    ...
    {
        ArvoreAVL *curso;
        curso = arvoreavl_buscar(raiz_curso, aluno->info.codigo_curso);

        int codigo_disciplina;
        leia_int("Código da disciplina: ", &codigo_disciplina);

        ArvoreAVL *disciplina;
        disciplina = arvoreavl_buscar(curso->info.curso.arvavl_disciplina, codigo_disciplina);

        if (disciplina != NULL)
        {
            union Data info;
            info.matricula = codigo_disciplina;
            if(arvoreavl_add(&(aluno->info.arvavl_matricula), info))
...
void cadastrar_notas(Lista *lista)
{
    ...
    if (aluno != NULL)
    {
        ...
        if(arvoreavl_remover(&(aluno->info.arvavl_matricula), nota.codigo_disciplina))
        {
            ...
            union Data info;
            info.nota = nota;
            if(arvoreavl_add(&(aluno->info.arvavl_nota), info))
